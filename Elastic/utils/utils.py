...
def load_generator(model_name, update_weights=None):
    ...
    Raises:
        ...
        :param update_weights: if None than load raw weights otherwise load updates weights
    """
    ...
    if 'generator_smooth' in checkpoint:
        if model_name == 'pggan_celebahq1024':
            modulate = {
                k: v
                for k, v in checkpoint['generator_smooth'].items()
                if "layer0" in k and "weight" in k
            }
            #
            # print(modulate['layer0.weight'].shape) (512, 512, 4, 4)
        else:
            modulate = {
                k: v
                for k, v in checkpoint['generator_smooth'].items()
                if "style" in k and "output" not in k and "weight" in k
            }
        if update_weights is None:
            generator.load_state_dict(checkpoint['generator_smooth'])

        else:
            # update_weights['layer0.weight'] = update_weights['layer0.weight'].view(512, 512, 4, 4)
            if model_name == 'pggan_celebahq1024':
                update_weights['layer0.weight'] = update_weights['layer0.weight'].permute(1, 0).view(512, 512, 4, 4).permute(1, 0, 2, 3).flip(2, 3)
            checkpoint['generator_smooth'].update(update_weights)
            generator.load_state_dict(checkpoint['generator_smooth'])
    else:
        modulate = {
            k: v
        ...
        if update_weights is None:
            ...
            checkpoint['generator'].update(update_weights)
            generator.load_state_dict(checkpoint['generator'])
    generator = generator.cuda()
    generator.eval()
    print(f'Finish loading checkpoint.')
    return generator, modulate


def load_generator_custom(model_name, update_weights=None):
    """Loads pre-trained generator.
    ...
    if 'g_ema' in checkpoint:
        ...
        if update_weights is None:
            ...
            checkpoint['g_ema'].update(update_weights)
            generator.load_state_dict(checkpoint['g_ema'])
    else:
        modulate = {
            k: v
        ...
        if update_weights is None:
            ...
            checkpoint['g'].update(update_weights)
            generator.load_state_dict(checkpoint['g'])
    generator = generator.cuda()
    generator.eval()
    print(f'Finish loading checkpoint.')
...
def factorize_weight(generator, layer_idx='all'):
    ...
    if gan_type == 'pggan':
        ...
        if layer_idx == 'all':
            ...
            print("Used layers of: ", layers)

    # Factorize semantics from weight.
    weights = []
    for idx in layers:
        layer_name = f'layer{idx}'
        # print("layer_name", layer_name)  layer0, layer1,...layer17
        if gan_type == 'stylegan2' and idx == generator.num_layers - 1:
            # layer_name = f'output{idx // 2}'
            break
        if gan_type == 'pggan':
            weight = generator.__getattr__(layer_name).weight  # (512, 512, 4, 4)
            weight = weight.flip(2, 3).permute(1, 0, 2, 3).reshape(512, -1)  # (512, 8192)
            weight = weight.permute(1, 0)  # (8192, 512)
        elif gan_type in ['stylegan', 'stylegan2']:
            weight = generator.synthesis.__getattr__(layer_name).style.weight
    ...
    # weight = np.concatenate(weights, axis=0).astype(np.float32)
    # weight = weight / np.linalg.norm(weight, axis=1, keepdims=True)

    U, S, V = torch.svd(weight)
    # U, S, V = np.linalg.svd(weight)
    # eigen_values, eigen_vectors = np.linalg.eig(weight.T.dot(weight))

    return layers, U, S, V  # eigen_vectors.T, eigen_values


def factorize_weight_SeFa(generator, layer_idx='all'):
    """Factorizes the generator weight to get semantics boundaries.
    Args:
        generator: Generator to factorize.
        layer_idx: Indices of layers to interpret, especially for StyleGAN and
